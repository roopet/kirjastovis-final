...
defaultOptions = {
	...
	credits: {
		enabled: true,
		text: 'Copyright Opetus- ja kulttuuriministeriö',
		href: 'http://www.vividin.fi',
		position: {
			align: 'right',
			x: -10,
			verticalAlign: 'bottom',
...
Point.prototype = {
	...
	firePointEvent: function (eventType, eventArgs, defaultFunction) {
		...
		// add default handler if in selection mode, different setup for mobile
		if ('ontouchstart' in document.documentElement) {
		if (eventType === 'click' && seriesOptions.allowPointSelect) {
			defaultFunction = function (event) {

				//
				point.select(null, 'click');
				var chart = $('#map-container').highcharts()
				chart.get("suomi").select(false, true);
			//	alternative: hide koko maa series
